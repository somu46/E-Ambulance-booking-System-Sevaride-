        ...
        <div className='container'>
            {!selectionMade && // Render user selection buttons if selection is not made
                <div className='user-container'>
                    <div className={action === "User" ? "submit gray" : "alluser"} onClick={handleUserClick} color='#324595'>User</div>
                    <div className={action === "Driver" ? "submit gray" : "alluser"} onClick={handleDriverClick} color='#e50614'>Driver</div>
                </div>
            }
            ...
            {selectionMade && // Render greeting if selection is made
                <div className='header'>
                    ...
                    <div className='greeting'>{action === 'User' ? 'Hello User' : 'Hello Partner'}</div>
            ...
            {selectionMade && // Render sign-up and login options if selection is made
                <div className='container-submit'>
                    <div className={action === "Sign Up" ? "submit gray" : "submit"} onClick={handleSignUpClick}>Sign Up</div>
                    <div className={action === "Login" ? "submit gray" : "submit"} onClick={handleLogInClick}>Log In</div>
                </div>
            }
                ...
                <div className='inputs'>
                    ...
                    {driverInputVisible &&  // Render additional inputs for driver
                        <div className='input'>
                            ...
                            <input type="text" className='inputBox' placeholder='Driver License' />
                        </div>
                    }
                </div>
            }

            {selectionMade && action !== "Sign Up" && // Render forgot password link if selection is made and action is not sign up
                <div className='fogot-password'>Forgot Password?<span>click here</span></div>
            }
        </div>
    )
}
